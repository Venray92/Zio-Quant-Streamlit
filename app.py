...
with col_right:
    ...
    st.markdown("<hr style='margin-top: 2px; margin-bottom: 10px; border-color: #1E2D3D;'>", unsafe_allow_html=True)
    ...
    if st.session_state.view_mode == "chart":
        ...
        if plan.get("is_ihsg"):
            st.info("Trade plan spesifik tidak tersedia untuk IHSG. Silakan pilih emiten saham tertentu dari daftar screener.")
        elif "error" in plan:
            ...
            with col_tp1:
                st.markdown(f"""
                <div class="tp-card">
                    <p style='color: #94A3B8; font-size: 12px; margin-bottom: 2px;'>STATUS & STRATEGI</p>
                    <p style='font-size: 16px; font-weight: bold; color: {'#00E676' if plan['rule_d_status']=='PASSED' else '#FF5252'}; margin-bottom: 6px;'>{plan['rule_d_status']}</p>
                    <p style='font-size: 14px; font-weight: bold; color: #E2E8F0;'>{plan['selected_strategy']}</p>
                    <p style='font-size: 11px; color: #94A3B8; margin-top: 4px;'>{plan['volume_note']}</p>
                </div>
                """, unsafe_allow_html=True)
            with col_tp2:
                st.markdown(f"""
                <div class="tp-card">
                    <p style='color: #94A3B8; font-size: 12px; margin-bottom: 2px;'>AREA ENTRY & TARGET</p>
                    <p style='font-size: 14px; font-weight: bold; color: #00B0FF; margin-bottom: 4px;'>Entry: {plan['entry_range']}</p>
                    <p style='font-size: 12px; color: #E2E8F0;'>Worst Case: {plan['worst_case_entry']}</p>
                </div>
                """, unsafe_allow_html=True)
            with col_tp3:
                st.markdown(f"""
                <div class="tp-card">
                    <p style='color: #94A3B8; font-size: 12px; margin-bottom: 2px;'>STOP LOSS & RISIKO</p>
                    <p style='font-size: 14px; font-weight: bold; color: #FF5252; margin-bottom: 4px;'>SL: {plan['sl_price']}</p>
                    <p style='font-size: 12px; color: #E2E8F0;'>Risiko: {plan['max_risk_pct']} ({plan['max_risk_status']})</p>
                </div>
                """, unsafe_allow_html=True)

            st.markdown("<p style='font-size: 14px; font-weight: bold; margin-top: 15px; margin-bottom: 10px;'>Target Profit (TP) & Risk/Reward Ratio</p>", unsafe_allow_html=True)
            
            for t in plan["targets"]:
                st.markdown(f"""
                <div class="tp-card-blue">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-weight: bold; font-size: 14px; color: #00B0FF;">{t['target_label']}</span>
                        <span class="tp-badge-gold">{t['target_price']}</span>
                    </div>
                    <p style="font-size: 12px; color: #94A3B8; margin: 4px 0;">Basis: {t['target_basis']} | Potensi Gain: <span style="color: #00E676; font-weight: bold;">{t['potential_gain_pct']}</span></p>
                    <p style="font-size: 12px; color: #E2E8F0; margin: 0;">R:R Ratio: <span style="font-weight: bold;">{t['rr_ratio']}</span> ({t['rr_status_label']})</p>
                </div>
                """, unsafe_allow_html=True)
